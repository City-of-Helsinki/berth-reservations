    ...
    def handle(self, **options):
        ...
        for harbor in Harbor.objects.all():
            with transaction.atomic():
                self.stdout.write(
                    'Fetching info for harbor with servicemap ID {}'.format(harbor.servicemap_id)
                )

                sm_response = requests.get(self._get_servicemap_url(harbor.servicemap_id))
                ...
                harbor.municipality = Municipality.objects.get(id=sm_info['municipality'])
