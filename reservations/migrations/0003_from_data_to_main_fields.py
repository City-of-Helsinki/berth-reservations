            ...
            with transaction.atomic():
                ...
                for index, harbor_identifier in enumerate(chosen_harbors, 1):
                    ...
                    if harbor:
                        ...
                        logger.error(
                            "Could not find harbor with identifier {}"
                            .format(harbor_identifier))

                boat_type = deep_get(r.data, "boat.type")
                if boat_type:
                    boat_type_obj = BoatType.objects.filter(identifier=boat_type).first()
                    if boat_type_obj:
...
class Migration(migrations.Migration):
    ...
    dependencies = [
        ('reservations', '0002_add_main_fields'),
    ]
